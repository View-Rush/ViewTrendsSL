...
const Channels = () => {
  const { channels, loadChannels, loading, error } = useChannelStore();
  ...
  if (error) {
    return (
        <div className="flex flex-col items-center justify-center h-screen space-y-3">
          ...
          <Button onClick={() => loadChannels(true)}>Retry</Button>
        </div>
    );
  }
            ...
            <CardContent className="pt-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <h3 className="text-2xl font-bold">
                    {channels
                        .reduce((sum, ch) => sum + (ch.view_count || 0), 0)
                        .toLocaleString()}
                  </h3>
                </div>
              </div>
          ...
          <CardContent>
            {channels.length === 0 ? (
                <div className="text-center text-muted-foreground py-12">
                  No channels found. Try syncing or adding one.
                </div>
            ) : (
                  ...
                  {channels.map((channel) => (
                      ...
                      >
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <div className="flex items-center gap-4">
                            <Badge
                                variant={channel.is_connected ? "default" : "secondary"}
                            >
                        ...
                        <span className="text-sm text-muted-foreground">
                          ...
                          {channel.view_count?.toLocaleString() || 0}
                            ...
                            <Progress
                                value={Math.min((channel.view_count || 0) / 1000000 * 100, 100)}
                                className="h-2"
                            />
                          </div>
                          <div>
                        ...
                        <span className="text-sm text-muted-foreground">
                          ...
                          {channel.video_count || 0}
                            ...
                            <Progress
                                value={Math.min((channel.video_count || 0) * 2, 100)}
                                className="h-2"
                            />
                          </div>
                        </div>
